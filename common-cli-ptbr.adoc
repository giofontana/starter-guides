...
OpenShift entrega uma rica interface web bem como uma ferramenta de linha de comando (CLI) para prover para usuários uma boa experiencia para trabalhar com a plataforma. As ferramentas são executáveis simples escritos em Go e disponível nos seguintes sistemas operacionais:

- Microsoft Windows
- macOS 10
- Linux

Você pode já ter um OpenShift CLI disponível no seu ambiente. Você pode verificar através do comando `oc`:
...
Se o `oc` não existir ou tenha uma versão antiga do OpenShift CLi, siga as instruções desta seção. Senão, vá para o próximo lab.
